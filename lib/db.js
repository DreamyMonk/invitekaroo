...
export async function createCommunity(uid, data) {
  // A host owns exactly ONE community. Guard against creating a duplicate:
  // if one already exists for this owner (e.g. a transient getMyCommunity
  // failure or a double call landed us here), return it instead of adding a
  // second same-named community — which would split subscribers across two
  // push topics and silently break notifications.
  const existing = await getMyCommunity(uid);
...
/* ─────────── Push (server route) ─────────── */
export async function pushNotify(title, body, cid, wa, push) {
  try {
    const r = await fetch("/api/notify", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, body, cid, wa, push }),
    });
    return await r.json();
  } catch (e) {
